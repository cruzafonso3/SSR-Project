...
static int arpCount = 0, dnsCount = 0, dhcpCount = 0, deauthCount = 0;
static int cursor = 0;
...
void loop() {
    ...
    if (!showStatus) {
        ...
        if (evt == EVT_SELECT) {
            ...
            if (cursor == 3) { deauthOn = !deauthOn; if (deauthOn) deauthStart(); else deauthStop(); }
...
void renderStatus() {
    ...
    displayDrawStr(24, 22, WiFi.status()==WL_CONNECTED ? WiFi.localIP().toString().c_str() : "--");
