...
static void sendDhcpReply(const uint8_t* clientMac, uint32_t xid, uint8_t msgType, const uint8_t* reqIpPkt) {
    ...
    // FIX: store checksum in network byte order (big-endian)
    ipHdr[10] = (csum >> 8) & 0xFF;
    ipHdr[11] = csum & 0xFF;
...
void dhcpProcessPacket(const uint8_t* ipPkt, const uint8_t* udpPkt, const uint8_t* dhcpData, uint16_t dhcpLen) {
    ...
    while (opt < dhcpLen) {
        ...
        if (code == 255) break;
        if (code == 0) continue;
        // FIX: bounds checking before reading length and data
        if (opt >= dhcpLen) break;
        uint8_t len = dhcpData[opt++];
        if (opt + len > dhcpLen) break;
